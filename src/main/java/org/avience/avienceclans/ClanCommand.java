...
// Confirm Delete command
                ...
                if (clansSection != null) {
                    for (String existingClan : clansSection.getKeys(false)) {
                        roleOfCommandSender = clanConfig.getString("clans." + existingClan + ".members." + player.getUniqueId() + ".role");
                ...
                if (clanConfig.contains("clans." + clanName + ".members." + targetUUID)) {
                ...
                if (clansSection != null) {
                    for (String existingClan : clansSection.getKeys(false)) {
                        String role = clanConfig.getString("clans." + existingClan + ".members." + player.getUniqueId() + ".role");
...
// Promote command
                ...
                if (clansSection != null) {
                    for (String existingClan : clansSection.getKeys(false)) {
                        ...
                        if (clanName != null) {
                            List<String> allMembers = clanConfig.getStringList("clans." + clanName + ".members");

                            for (String memberUUID : allMembers) {
                                Player memberPlayer = Bukkit.getPlayer(UUID.fromString(memberUUID));
                                if (memberPlayer != null && memberPlayer.isOnline()) {
                                    plugin.sendPrefixedMessage(memberPlayer, playerNameToPromote + " has been promoted in clan " + clanName + ".");
                                }
                            }
                        }
                    }
                    ...
                    if (matchingClan.isPresent()) {
                        ...
                        if (invites.contains(playerUUID.toString())) {  // Changed to UUID
                            ...
                            if (!members.contains(playerUUID.toString())) {
                                ...
                                for (String memberUUID : members) {
                                    Player memberPlayer = Bukkit.getPlayer(UUID.fromString(memberUUID));
                                    if (memberPlayer != null && memberPlayer.isOnline()) {  // Make sure the player is online
                                        plugin.sendPrefixedMessage(memberPlayer, player.getName() + " has joined your clan!");
                                    }
                                }
...
// Kick command
                    ...
                    if ("leader".equalsIgnoreCase(roleOfCommandSender) ||
                        ...
                        if (clanSection != null) {
                            ...
                            // Inform the player that they have kicked someone
                            ...
                            // Inform the entire clan that a member has been kicked
                            for (String memberUUID : members) {
                                Player memberPlayer = Bukkit.getPlayer(UUID.fromString(memberUUID));
                                if (memberPlayer != null && memberPlayer.isOnline()) {  // Make sure the player is online
                                    plugin.sendPrefixedMessage(memberPlayer, playerNameToKick + " has been kicked from your clan!");
                                }
                            }
                        }
                    } else {
...
// Demote command
                    ...
                    if (admins.contains(uuidToDemote.toString())) {
                        if ("leader".equals(senderRole)) {
                            ...
                            // Notify all members of the clan about the demotion
                            for (String memberUUID : members) {
                                Player memberPlayer = Bukkit.getPlayer(UUID.fromString(memberUUID));
                                if (memberPlayer != null && memberPlayer.isOnline()) {  // Make sure the player is online
                                    plugin.sendPrefixedMessage(memberPlayer, playerNameToDemote + " has been demoted to moderator in your clan!");
                                }
                            }
                        } else {
                        ...
                        for (String memberUUID : members) {
                            Player memberPlayer = Bukkit.getPlayer(UUID.fromString(memberUUID));
                            if (memberPlayer != null && memberPlayer.isOnline()) {  // Make sure the player is online
                                plugin.sendPrefixedMessage(memberPlayer, playerNameToDemote + " has been demoted to member in your clan!");
                            }
                        }
                    } else {

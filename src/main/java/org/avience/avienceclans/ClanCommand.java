...
// Delete command
        if ("delete".equalsIgnoreCase(args[0])) {
            ...
            if (clansSection != null) {
                for (String existingClan : clansSection.getKeys(false)) {
                    if (!player.getUniqueId().toString().equals(clanConfig.getString("clans." + clanName + ".leader"))) {
...
// Promote command
        ...
        if ("join".equalsIgnoreCase(args[0])) {
            ...
            if (clansSection != null) {
                System.out.println("Debug: clansSection is not null");
                ...
                System.out.println("Debug: matchingClan.isPresent() = " + matchingClan.isPresent());

                if (matchingClan.isPresent()) {
                    List<String> invites = clanConfig.getStringList("clans." + matchingClan.get() + ".invites");
                    System.out.println("Debug: Inside matchingClan.isPresent()");
                    ...
                    if (invites.contains(playerUUID.toString())) {  // Changed to UUID
                        ...
                        // Debug log
                        System.out.println("Debug: Invites list: " + invites);
                        System.out.println("Debug: Player UUID: " + playerUUID.toString());
                    ...
                    System.out.println("Debug: Inside else block of matchingClan.isPresent()");
...
// Transfer command
            if ("transfer".equalsIgnoreCase(args[0])) {
                ...
                Player playerToTransfer = Bukkit.getPlayer(playerNameToTransfer);
                if (playerToTransfer == null) {
                    plugin.sendPrefixedMessage(player, "The specified player is not online.");
                    return true;
                }
                UUID uuidToTransfer = playerToTransfer.getUniqueId(); // Convert player name to UUID
                ...
                for (String existingClan : clansSection.getKeys(false)) {
                    ...
                    if (uuidToTransfer.toString().equals(leader) || members.contains(uuidToTransfer.toString()) || mods.contains(uuidToTransfer.toString()) || admins.contains(uuidToTransfer.toString())) {
                ...
                clanConfig.set("clans." + clanName + ".leader", uuidToTransfer.toString());
                plugin.saveClanFile();
            ...
            if ("info".equalsIgnoreCase(args[0])) {
                ...
                player.sendMessage(ChatColor.AQUA + "Leader: " + (leader != null ? leader.getName() : "Unknown"));
                ...
                for (String memberUUID : moderators) {
                    Player member = Bukkit.getPlayer(UUID.fromString(memberUUID));
                    if (member != null) {
                        player.sendMessage(ChatColor.AQUA + "Member: " + member.getName() + " | Rank: " + ChatColor.GREEN + "Moderator");
                    }
                }
                ...
                for (String memberUUID : admins) {
                    Player member = Bukkit.getPlayer(UUID.fromString(memberUUID));
                    if (member != null) {
                        player.sendMessage(ChatColor.AQUA + "Member: " + member.getName() + " | Rank: " + ChatColor.GREEN + "Admin");
                    }
                }
            }
